...
void AsynClient::Connect(const std::string ip, WORD port, PAsynServer& iocp, PAsynClient& self)
{
	m_pServer = iocp;
	m_self = self;
	m_socket = WSASocketW(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
	bind(m_socket, m_addr, m_addr.size());
	self->m_addr.sin_family = PF_INET;
	self->m_addr.sin_port = htons(port);
	inet_pton(AF_INET, (PSTR)ip.c_str(), &self->m_addr.sin_addr);
	m_pServer->AddClient(self);
}
...
int AsynClient::Send(const PCPacket& packet)
{
	...
	if (ret != 0) {
		TRACEW("ret = %d m_socket=%d error=%d WSA_IO_PENDING(%d)",
...
AsynServer::AsynServer(const std::string& sIP, short port)
{
	...
	if (ret != 0) {
		TRACEE("bind 失败！ret = %d errno = %d\r\n", ret, WSAGetLastError());
		return;
	}
	ret = listen(m_socket, 10);
	if (ret != 0) {
		TRACEE("listen 失败！ret = %d errno = %d\r\n", ret, WSAGetLastError());
	}
	HANDLE handle = CreateIoCompletionPort((HANDLE)m_socket, m_iocp, NULL, 0);
	if (handle == INVALID_HANDLE_VALUE) {
		TRACEE(_T("CreateIoCompletionPort GetLastError = %d"), GetLastError());
	}
	m_ip = sIP;
	m_port = port;
}
...
int AsynServer::Start(PAsynServer& self, PBLayer& business)
{
	...
	ret = m_Task.Run();
	if (ret != 0) {
		TRACEE(_T("Run ret = %d"), ret);
	}
	ret = m_Task.AddTask(Task(&AsynServer::Accept, this));
	if (ret != 0) {
		TRACEE(_T("AddTask失败 ret = %d"), ret);
	}
...
int AsynServer::AddClient(PAsynClient& client)
{
	LPFN_CONNECTEX ConnectEx = NULL;
	DWORD dwBytes = 0;
	GUID guidConnectEx = WSAID_CONNECTEX;
	WSAIoctl((SOCKET)*client, SIO_GET_EXTENSION_FUNCTION_POINTER, &guidConnectEx, sizeof(guidConnectEx),
	...
	BOOL ret = ConnectEx(*client,
		*client, sizeof(sockaddr_in), NULL, 0, &dwBytes, &olp->overlapped);
	if (ret == FALSE) {
		if (WSAGetLastError() != WSA_IO_PENDING) {
			TRACEW(_T("ConnectEx WSAGetLastError = %d WSA_IO_PENDING(%d)"), WSAGetLastError(), WSA_IO_PENDING);
		}
	}
	return 0;
}
...
int AsynServer::Accept()
{
	...
	CreateIoCompletionPort((HANDLE)(SOCKET)*client, m_iocp, NULL, NULL);
	OpOL* olp = new OpOL(client, 4);
	olp->packet = std::make_shared<CPacket>(CPacket(128));
	TRACED(_T("client = %d"), (SOCKET)*client);
	...
	if (ret == FALSE) {
		if (WSAGetLastError() != WSA_IO_PENDING) {
			TRACEW(_T("AcceptEx WSAGetLastError = %d WSA_IO_PENDING(%d)"), WSAGetLastError(), WSA_IO_PENDING);
		}
	}
	return 0;
}
	...
	if (GetQueuedCompletionStatus(m_iocp, &NOBT, &CK, &Overlapped, INFINITE)) {
		if (Overlapped != 0) {
			...
			case 4://连接成功
				...
				m_mapClients.AddClient(opl->client);//加入客户端映射表
				opl->client->Recv();//客户端添加关注接收
				m_business->ClientConnected(opl->client);
				break;
			}
		}
	}
