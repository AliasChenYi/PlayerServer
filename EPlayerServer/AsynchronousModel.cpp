...
public:
	...
	virtual int ClientConnected(PAsynClient& client) {
		TRACEI(_T("ip %s port %u"), client->Ip().c_str(), client->Port() & 0xFFFF);
		return 0;
	};
	//客户端收到数据了，不要阻塞
...
TEST(AsynSocket, Server)
{
	PAsynServer server(new AsynServer("127.0.0.1"));
	PBLayer bussiness(new TestBussiness());
	int ret = server->Start(server, bussiness);
	EXPECT_EQ(ret, 0);
	SOCKET s = socket(AF_INET, SOCK_STREAM, 0);
	EXPECT_NE(INVALID_SOCKET, s);
	SockAddrIn addr;
	addr.sin_port = htons(8112);
	inet_pton(AF_INET, _T("127.0.0.1"), &addr.sin_addr);
	ret = connect(s, (sockaddr*)&addr, sizeof(sockaddr_in));
	TRACE("WSA error = %d", WSAGetLastError());
	EXPECT_EQ(ret, 0);
