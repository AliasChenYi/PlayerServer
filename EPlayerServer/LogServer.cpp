...
TEST(ServerTest, LogTest)
{
	TRACEI(_T("test ASCII output"));
	TRACED(_T("0123456789 ~!@#$%%^&*()_+{}[]:;\"'|\\,.<>?/"));
	TRACE(_T("测试中文输出"));
	TRACEW(_T("警告级别测试 %d %s %f %s"), 100, _T("哈哈abcdef"), 100.1234f, _T("0123456789"));
	TRACEE(_T("错误级别测试 %d %s %f %s"), 100, _T("哈哈abcdef"), 100.1234f, _T("0123456789"));
	TRACEF(_T("致命级别测试 %d %s %f %s"), 100, _T("哈哈abcdef"), 100.1234f, _T("0123456789"));
	const char data[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ哈哈中文你好";
	DUMP(data, sizeof(data));
	DUMPI(data, sizeof(data));
	DUMPD(data, sizeof(data));
	DUMPW(data, sizeof(data));
	DUMPE(data, sizeof(data));
	DUMPF(data, sizeof(data));
	ELOG << _T("hello");
	ELOG << _T("你好") << 100 << 100.0f << 111.11 << true << tstring(_T("哈哈哈"));
}

#define ALLOC_SIZE 100000
void thread_func() {
	//初始化，准备数据
	std::vector<unsigned> m_vecSize;
	m_vecSize.resize(ALLOC_SIZE);
	srand((unsigned int)time(NULL));
	for (int i = 0; i < ALLOC_SIZE; i++) {
		m_vecSize[i] = 0xFFFF & rand();
	}
	...
	tick = GetTickCount64();
	for (int i = 0; i < ALLOC_SIZE; i++) {
		p0 = malloc(m_vecSize[i]);
		p1 = malloc(m_vecSize[i] / 2);
		free(p0);
		free(p1);
	}
	TRACED("[ RUN=====>] malloc run time = %d\n", DWORD(GetTickCount64() - tick));
	tick = GetTickCount64();
	for (int i = 0; i < ALLOC_SIZE; i++) {
		p0 = je_malloc(m_vecSize[i]);
		p1 = je_malloc(m_vecSize[i] / 2);
		je_free(p0);
		je_free(p1);
	}
	TRACED("[ RUN=====>] jemalloc run time = %d\n", DWORD(GetTickCount64() - tick));

}

TEST(ServerTest, JemallocTest)
{
